...
describe('asn1.js RFC5280', function () {
  it('should decode Certificate', function () {
    const data = fs.readFileSync(path.join(__dirname, '/fixtures/cert1.crt'))
    const res = rfc5280.Certificate.decode(data, 'der')

    const tbs = res.tbsCertificate
    assert.equal(tbs.version, 'v3')
    assert.deepEqual(tbs.serialNumber, BigInt('0x462e4256bb1194dc'))
  ...
  it('should decode ECC Certificate', function () {
    ...
    const data = fs.readFileSync(path.join(__dirname, '/fixtures/cert2.crt'))
    const res = rfc5280.Certificate.decode(data, 'der')

    const tbs = res.tbsCertificate
    assert.equal(tbs.version, 'v3')
    assert.deepEqual(tbs.serialNumber, BigInt('0x4d955d20af85c49f6925fbab7c665f89'))
    assert.equal(tbs.signature.algorithm.join('.'),
  ...
  it('should decode Certificate Extensions', function () {
    ...
    let extensions = {}
    data = fs.readFileSync(path.join(__dirname, '/fixtures/cert3.crt'))
    ...
    extensions = {}
    data = fs.readFileSync(path.join(__dirname, '/fixtures/cert4.crt'))
    cert = rfc5280.Certificate.decode(data, 'der')
    cert.tbsCertificate.extensions.forEach(function (e) {
      extensions[e.extnID] = e
    })
    ...
    extensions = {}
    data = fs.readFileSync(path.join(__dirname, '/fixtures/cert5.crt'))
    cert = rfc5280.Certificate.decode(data, 'der')
    cert.tbsCertificate.extensions.forEach(function (e) {
      extensions[e.extnID] = e
    })
    ...
    extensions = {}
    data = fs.readFileSync(path.join(__dirname, '/fixtures/cert6.crt'))
    cert = rfc5280.Certificate.decode(data, 'der')
    cert.tbsCertificate.extensions.forEach(function (e) {
      extensions[e.extnID] = e
    })
  ...
  it('should decode Revoked Certificates', function () {
    ...
    data = fs.readFileSync(path.join(__dirname, '/fixtures/cert1.crl'))

    crl = rfc5280.CertificateList.decode(data, 'der')
    assert.equal(crl.tbsCertList.revokedCertificates.length, 2)
    assert.deepEqual(crl.tbsCertList.revokedCertificates[0].userCertificate, BigInt('0x764bedd38afd51f7'))

    const cert1 = crl.tbsCertList.revokedCertificates[1]
    assert.deepEqual(cert1.userCertificate, BigInt('0x31da3380182af9b2'))
    ...
    data = fs.readFileSync(path.join(__dirname, '/fixtures/cert4.crl'))
