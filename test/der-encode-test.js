...
describe('asn1.js DER encoder', function () {
  ...
  function test (name, modelDefinition, modelValue, derExpected) {
    it(name, function () {
      const Model = asn1.define('Model', modelDefinition)
      const derActual = Model.encode(modelValue, 'der')
      assert.deepEqual(derActual, Buffer.from(derExpected, 'hex'))
    })
  }
